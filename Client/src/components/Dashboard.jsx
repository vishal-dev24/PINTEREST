...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
            {/* Navbar */}
            <nav className="bg-white dark:bg-gray-800 drop-shadow-md p-2 sticky top-0 z-50 mb-6 rounded-xl">
                <div className="flex justify-between items-center px-2">
                    <button onClick={() => navigate("/home")} className="flex items-center space-x-2 text-2xl font-bold">
                        ...
                        {data.posts.map((post) => (
                            <div key={post._id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-2 hover:shadow-xl transition-all">
                                ...
                                <div className="mt-2 flex justify-between items-center">
                                    <button onClick={() => editPost(`/edit/${post._id}`)} className="text-blue-500 hover:text-blue-700 text-sm">Edit</button>
                                    <button onClick={() => deletePost(post._id)} className="text-red-500 hover:text-red-700 text-sm">Delete</button>
                                </div>
                            </div>
                        ))}
                    </div>
